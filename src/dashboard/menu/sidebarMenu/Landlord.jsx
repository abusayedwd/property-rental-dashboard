...
import { useGetAllLandlordQuery } from "../../../redux/features/user/getAllLandlord";
import { useBlockUnblockUserMutation } from "../../../redux/features/user/blockUnblockUser";
...
const [user, setUser] = useState()
 const {data: userss} =  useGetAllLandlordQuery(name)
...

    {
      ...
      render: (_, record) => (
        <Space size="middle">
          ...
          />
          <button
            onClick={() => handleBlockUnblock(record?.id)} // Replace with your block/unblock handler function
            className={`${
              record.isBlocked ? 'bg-[#c7b98f]' : 'bg-[#CD5C5C]'
            } text-[#FFFF] cursor-pointer py-2 px-4 rounded`}
          >
    ...
    <div className="">
      ...
      <div className="rounded-t-lg mt-[24px]">
        <div className="flex py-[22px] justify-between items-center">
          ...
          <p className="text-header">Users</p>
          </div>
          <div>
